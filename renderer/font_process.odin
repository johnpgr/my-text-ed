...
process :: proc(font: ^Font, allocator := context.allocator) -> Texture_Pack_Result {
	for _, &g in font.glyphs {
		if g.valid && len(g.curves) > 0 {
			glyph_process(&g, allocator)
		}
	}
	return pack_glyph_textures(font, allocator)
}

@(private = "file")
glyph_process :: proc(g: ^Glyph_Data, allocator := context.allocator) {
	...
	h_lists := make([][dynamic]u16, h_band_count, allocator)
	defer {
		for &l in h_lists do delete(l)
		delete(h_lists)
	}
	v_lists := make([][dynamic]u16, v_band_count, allocator)
	defer {
	...
	for ci in 0 ..< num_curves {
		...
		min_y := min(curve.start.y, curve.control.y, curve.end.y) - BAND_EPSILON
		max_y := max(curve.start.y, curve.control.y, curve.end.y) + BAND_EPSILON
		min_x := min(curve.start.x, curve.control.x, curve.end.x) - BAND_EPSILON
		max_x := max(curve.start.x, curve.control.x, curve.end.x) + BAND_EPSILON
...
sort_curve_indices_by_max_x :: proc(indices: []u16, curves: []Bezier_Curve) {
	for i := 1; i < len(indices); i += 1 {
		key := indices[i]
		key_max_x := max(curves[key].start.x, curves[key].control.x, curves[key].end.x)
		j := i - 1
		for j >= 0 {
			j_max_x := max(
				curves[indices[j]].start.x,
				curves[indices[j]].control.x,
				curves[indices[j]].end.x,
			)
...
sort_curve_indices_by_max_y :: proc(indices: []u16, curves: []Bezier_Curve) {
	for i := 1; i < len(indices); i += 1 {
		key := indices[i]
		key_max_y := max(curves[key].start.y, curves[key].control.y, curves[key].end.y)
		j := i - 1
		for j >= 0 {
			j_max_y := max(
				curves[indices[j]].start.y,
				curves[indices[j]].control.y,
				curves[indices[j]].end.y,
			)
...
@(private = "file")
pack_glyph_textures :: proc(font: ^Font, allocator := context.allocator) -> (result: Texture_Pack_Result) {
	curve_x: u32 = 0
	curve_y: u32 = 0
	band_x: u32 = 0
	band_y: u32 = 0
	...
	for _, &g in font.glyphs {
		...
		for &curve in g.curves {
			append(
				&result.curve_data,
				[4]u16 {
					f32_to_f16(curve.start.x),
					f32_to_f16(curve.start.y),
					f32_to_f16(curve.control.x),
					f32_to_f16(curve.control.y),
				},
			)
			append(
				&result.curve_data,
				[4]u16{f32_to_f16(curve.end.x), f32_to_f16(curve.end.y), 0, 0},
			)
			curve_x += 2
		}
